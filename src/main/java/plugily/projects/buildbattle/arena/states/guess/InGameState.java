...
//round delay
  ...
  private void setChosenTheme(GuessArena pluginArena, BBTheme bbTheme) {
    ...
    pluginArena.getCurrentBuilders().forEach(player ->
        getPlugin().getActionBarManager().addActionBar(player, new ActionBar(new MessageBuilder("IN_GAME_MESSAGES_PLOT_GTB_THEME_NAME").asKey().value(pluginArena.getCurrentBBTheme().getTheme()).player(player).arena(pluginArena),
